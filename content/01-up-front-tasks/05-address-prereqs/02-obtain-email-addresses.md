...
|**`workload-test-<workload group identifier>`**|Formal test environments for your business-oriented workloads and data.<br><br>You should 1) identify how many distinct groups of related workloads you'll initially be supporting and 2) arrive at an AWS account name and email address for each distinct group.<br><br>For example `workloads-test-sap`, and `workloads-prod-sap`.|`aws-account-workload-test-<workload-group-identifier>@example.com`|
|**`workload-prod-<workload group identifier>`**|Environment where your initial production workloads and data will reside.|`aws-account-workload-prod-<workload-group-identifier>@example.com`|